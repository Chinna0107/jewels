...
export function CountryPicker({ dark, allowedCountries = [], value, onChange }) {
      ...
      <button
        ...
        className={`w-full flex items-center gap-2 px-4 py-3.5 rounded-xl text-sm border transition-all focus:outline-none ${
          dark
            ...
            : 'bg-white border-gray-200 text-gray-900 focus:ring-2 focus:ring-gray-400 bg-gray-50'
        }`}
      >
        {selected ? (
          ...
          <span className={`flex-1 text-left ${dark ? 'text-white/30' : 'text-gray-400'}`}>Select your country</span>
        )}
        <svg className={`w-4 h-4 shrink-0 transition-transform ${countryOpen ? 'rotate-180' : ''} ${dark ? 'text-white/40' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
      </button>
      {countryOpen && (
