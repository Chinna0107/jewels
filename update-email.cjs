...
const backendDir = '/Users/hemanthkancharla/jewelsbe';
const emailJsPath = path.join(backendDir, 'utils', 'email.js');
const authJsPath = path.join(backendDir, 'routes', 'auth.js');
const generalJsPath = path.join(backendDir, 'routes', 'general.js');

let emailJsContent = fs.readFileSync(emailJsPath, 'utf8');

if (!emailJsContent.includes('sendOrderEmailToCustomer')) {
    const customerEmailFunc = `
async function sendOrderEmailToCustomer(orderNumber, total, address, items, customerEmail, subtotal, discount, shipping, tax) {
  try {
    const addr = address || {};
    ...
    let attachments = [];
    const careGuidePath = require('path').join(__dirname, '..', 'public', 'jewelry-care-guide.jpg');
    if (require('fs').existsSync(careGuidePath)) {
        attachments.push({
            filename: 'Houra Jewels – Jewelry Care Guide.jpg',
            path: careGuidePath
        });
    }
...
`;
    emailJsContent = emailJsContent.replace('module.exports = { transporter, sendOrderEmailToAdmin, sendRefundEmail };', customerEmailFunc + '\nmodule.exports = { transporter, sendOrderEmailToAdmin, sendOrderEmailToCustomer, sendRefundEmail };');
...
const updateRoute = (filePath) => {
    ...
    if (!content.includes('sendOrderEmailToCustomer')) {
        content = content.replace(/const \{([^}]+)\} = require\('\.\.\/utils\/email'\);/, (match, p1) => {
            if (!p1.includes('sendOrderEmailToCustomer')) {
                return 'const { ' + p1.trim() + ', sendOrderEmailToCustomer } = require(\'../utils/email\');';
            }
            return match;
        });
        
        content = content.replace(/sendOrderEmailToAdmin\(([^)]+)\);/g, (match, args) => {
            return match + '\n    sendOrderEmailToCustomer(' + args + ', req.user?.email || req.body?.email || req.body?.customerEmail || address?.email, total, 0, 0, 0);';
        });
        fs.writeFileSync(filePath, content);
    }
}

updateRoute(authJsPath);
updateRoute(generalJsPath);

console.log("Updated email templates and routes successfully.");
